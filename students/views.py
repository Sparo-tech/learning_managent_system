...
from accounts.models import Teacher,User,Lecture_session, Resource,Assignment,AssignmentSubmission,Course
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Course.objects.filter(code=code).exists():
                ...
                if request.user.students.courses.filter(code=code).exists():
                    print('User already a member')
                    return HttpResponseRedirect(reverse('students:dashboard'))
                else:
                    print('User not a member')
                    request.user.students.courses.add(Course.objects.filter(code=code).first())
...
def LeaveClassroomView(request,code):
    request.user.students.classrooms.remove(Course.objects.filter(code=code).first())
    return HttpResponseRedirect(reverse('students:dashboard'))
