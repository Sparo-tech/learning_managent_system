...
from accounts.models import Course,Teacher,User,Lecture_session, Resource,Assignment,AssignmentSubmission
...
class DashboardView(TeacherTestMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['classrooms'] = self.request.user.teachers.courses.all()
        return context
...
def ClassroomDetailView(request,code):
    context_dict = {}
    room = get_object_or_404(Course, code=code)
    context_dict['course'] = room
...
class SectionCreateView(CreateView):
    ...
    def form_valid(self, form):
        form.instance.classroom = get_object_or_404(Course, code=self.kwargs['code'])
        return super().form_valid(form)
...
def ResourcesView(request, pk):
    context_dict = {}
    section = get_object_or_404(Lecture_session, pk=pk)
    print(section)
...
class ResourceCreateView(CreateView):
    ...
    def form_valid(self, form):
        form.instance.section = get_object_or_404(Lecture_session, pk=self.kwargs['pk'])
        return super().form_valid(form)
...
def ClassroomStudentsView(request,code):
    context_dict = {}
    classroom = get_object_or_404(Course, code=code)
